...
it is hard coded to accept only 1 user arguement and if none present default to testquiz
	...
	Example:
	within "def sbcontents(self,site):" is #server_name , uncomment this to have this script base its creation off of hostheader using regex lookups
	you will currently get warnings about dual port 8888 blocks if you test this without that consideration 
	as multiple blocks are attempting to listen for the same thing on the same port , as they are looking at seperate roots also you would have seperate html content
	in certain scenarios knowing and being able to introduce scenarios like this is beneficial.
